...
const constants = [
  ["a", "抗持续系数 a", 0.08, 0, 0.12],
  ["bCoef", "身体负荷系数 b", 0.05, 0, 0.10],
  ["cCoef", "认知占用系数 c", 0.04, 0, 0.10],
  ["d", "可控抗修正 d", 0.04, 0, 0.08],
  ["e", "柠持续系数 e", 0.06, 0, 0.12],
  ["f", "意义增益系数 f", 0.05, 0, 0.10],
  ["gCoef", "身体舒展系数 g", 0.03, 0, 0.08],
  ["h", "可控柠修正 h", 0.03, 0, 0.08],
];

const HISTORY_KEY = "kangning-calculator-history-v1";
let memoryHistory = [];
...
function renderInputs() {
  ...
  constants.forEach(([id, label, value, min, max]) => {
...
function updateOutputs() {
  ...
  const rangeHint = Math.abs(result.kn) > 50 ? "已超出仪表量程，仅指针封顶" : result.kn < 0 ? "净值偏抗" : result.kn > 0 ? "净值偏柠" : "净值平衡";
  ...
  elements.scoreHint.textContent = rangeHint;
...
async function copyResult() {
  try {
    ...
    setButtonText(elements.copyBtn, "已复制", "复制结果");
  } catch {
    elements.resultText.focus();
    elements.resultText.select();
    setButtonText(elements.copyBtn, "请手动复制", "复制结果", 1800);
  }
}

function setButtonText(button, temporaryText, originalText, delay = 1200) {
...
function currentRecord() {
  const result = calculate();
  const [label, text] = classify(result.kn);
  return {
    id: globalThis.crypto?.randomUUID ? crypto.randomUUID() : String(Date.now()),
    time: new Date().toISOString(),
    event: elements.eventName.value || "一次未命名体验",
    k: Number(fmt(result.k)),
    n: Number(fmt(result.n)),
    kn: Number(fmt(result.kn)),
    state: label,
...
function saveHistory(history) {
  memoryHistory = history.slice(0, 50);
  try {
    localStorage.setItem(HISTORY_KEY, JSON.stringify(memoryHistory));
  } catch {
    // Some local or embedded browsers disable localStorage; keep records for this page session.
  }
}

function saveCurrentRecord() {
  try {
    const history = [currentRecord(), ...loadHistory()];
    saveHistory(history);
    renderHistory();
    setButtonText(elements.saveBtn, "已保存", "保存到历史");
  } catch (error) {
    console.error("保存历史记录失败", error);
    setButtonText(elements.saveBtn, "保存失败", "保存到历史", 1800);
  }
}
...
function renderHistory() {
  ...
  history.forEach((record) => {
    ...
    li.innerHTML = `<strong>${escapeHtml(record.event)}</strong>：KN ${record.kn}，${escapeHtml(record.state)} <span>(${time})</span>`;
...
function clearHistory() {
  ...
  try {
    ...
    // Session-only fallback is already cleared above.
  }
  renderHistory();
}
...
function exportCsv() {
  try {
    ...
    if (history.length === 0) {
      setButtonText(elements.csvBtn, "暂无历史", "导出 CSV");
      return;
    }
    const header = ["time", "event", "k", "n", "kn", "state", "confidence", "interpretation"];
    const rows = history.map((record) => header.map((key) => csvCell(record[key])).join(","));
    const csv = `\uFEFF${header.join(",")}\n${rows.join("\n")}`;
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "抗柠指标历史记录.csv";
    a.click();
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error("导出 CSV 失败", error);
    setButtonText(elements.csvBtn, "导出失败", "导出 CSV", 1800);
  }
}
...
elements.saveBtn.addEventListener("click", saveCurrentRecord);
elements.csvBtn.addEventListener("click", exportCsv);
elements.clearHistoryBtn.addEventListener("click", clearHistory);
if (typeof window !== "undefined") {
  window.kangningActions = {
    calculate,
    saveCurrentRecord,
    exportCsv,
    clearHistory,
